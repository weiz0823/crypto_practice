    ...
    do {
        ...
        for (uint8_t i = 0; i < 80; ++i) {
            if (i < 16) {
                ...
                w[i] = w[i - 16] + w[i - 7];
                w[i] += RightRotate64(w[i - 15], 1) ^
                        RightRotate64(w[i - 15], 8) ^ (w[i - 15] >> 7);
                w[i] += RightRotate64(w[i - 2], 19) ^
                        RightRotate64(w[i - 2], 61) ^ (w[i - 2] >> 6);
            }
            f1 = k[i] + w[i] + a[7];
...
uint64_t SHA2::SHA512t(std::FILE* file, uint8_t* dest, uint32_t t) {
    ...
    for (uint8_t j = t - 1 - 8 * i; j != uint8_t(-1); --j) {
        dest[8 * i + j] = a[i] & 255;
        a[i] >>= 8;
    }
    return uint64_t(msg_len);
